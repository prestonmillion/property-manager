...
import { Home, Users, DollarSign, Shield, Plus, Search, ChevronRight, Edit2, AlertTriangle, CheckCircle, Clock, Building, Building2, Moon, Sun, ShieldCheck, ShieldX, X, ChevronDown, Landmark, Receipt } from 'lucide-react';
...
const paymentMethods = ['Zelle', 'CashApp', 'Venmo', 'Check', 'Direct Deposit', 'Zillow', 'Money Order', 'Cash', 'Other'];

const expenseCategories = ['Repairs', 'Maintenance', 'Capital Improvement', 'Insurance', 'Property Tax', 'HOA', 'Utilities', 'Property Management', 'Legal', 'Advertising', 'Supplies', 'Other'];

const expenses = [
  { id: 1, propId: 1, date: '2024-12-15', amount: 450, category: 'Repairs', payee: 'ABC Plumbing', method: 'Check', description: 'Fixed leaking water heater', notes: 'Annual inspection recommended' },
  { id: 2, propId: 1, date: '2024-08-20', amount: 8500, category: 'Capital Improvement', payee: 'Cool Air HVAC', method: 'Check', description: 'New HVAC system installed', notes: '10 year warranty, registered with manufacturer' },
  { id: 3, propId: 2, date: '2025-01-05', amount: 150, category: 'HOA', payee: 'Maple Avenue HOA', method: 'Direct Deposit', description: 'Monthly HOA dues - January', notes: '' },
  { id: 4, propId: 2, date: '2024-11-10', amount: 275, category: 'Maintenance', payee: 'Green Lawn Care', method: 'Zelle', description: 'Fall cleanup and leaf removal', notes: 'Quarterly service' },
  { id: 5, propId: 3, date: '2024-10-01', amount: 1200, category: 'Repairs', payee: 'Roof Masters', method: 'Check', description: 'Repaired roof leak over bedroom', notes: 'May need full replacement in 2-3 years' },
  { id: 6, propId: 4, date: '2025-01-20', amount: 2800, category: 'Maintenance', payee: 'Fresh Start Painting', method: 'Check', description: 'Full interior paint - turnover', notes: 'Sherwin Williams Agreeable Gray throughout' },
  { id: 7, propId: 4, date: '2025-01-22', amount: 1950, category: 'Maintenance', payee: 'Carpet World', method: 'Check', description: 'New carpet in bedrooms and stairs', notes: 'Receipt saved for taxes' },
  { id: 8, propId: 5, date: '2024-09-15', amount: 325, category: 'Repairs', payee: 'Handy Dan', method: 'CashApp', description: 'Fixed garbage disposal Unit A', notes: '' },
  { id: 9, propId: 5, date: '2024-06-01', amount: 185, category: 'Maintenance', payee: 'Pest Control Pro', method: 'Zelle', description: 'Quarterly pest treatment', notes: 'Both units' },
  { id: 10, propId: 3, date: '2024-07-15', amount: 95, category: 'Supplies', payee: 'Home Depot', method: 'Credit Card', description: 'Air filters, smoke detector batteries', notes: 'Stock for all properties' },
];
...
export default function App() {
  ...
  const [expenseForm, setExpenseForm] = useState({ propId: '', date: new Date().toISOString().split('T')[0], amount: '', category: 'Repairs', payee: '', method: 'Check', description: '', notes: '' });
  ...
  const recordExpense = () => {
    if (!expenseForm.propId || !expenseForm.amount || !expenseForm.category || !expenseForm.payee) return;
    
    const newExpense = {
      id: Math.max(...allExpenses.map(e => e.id)) + 1,
      propId: parseInt(expenseForm.propId),
    ...
    setExpenseForm({ propId: '', date: new Date().toISOString().split('T')[0], amount: '', category: 'Repairs', payee: '', method: 'Check', description: '', notes: '' });
  };
                    ...
                    {filtered.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className={`${th.hov} cursor-pointer`} onClick={() => setSelId(p.id)}>
                          <td className={`px-5 py-4 ${th.mut}`}>{p.id}</td>
                          <td className="px-5 py-4">
                            <p className={`text-sm font-medium ${th.txt}`}>{p.addr}</p>
                            <p className={`text-xs ${th.mut} mt-0.5`}>{p.city}, {p.state} {p.zip}</p>
                            <p className={`text-xs ${th.mut}`}>ID: {p.id}</p>
                          </td>
          ...
          {view === 'properties' && prop && (() => {
            ...
            <div className="space-y-6">
                  ...
                  <div className="flex items-center gap-6">
                    ...
                    <div>
                      <div className="flex items-center gap-3">
                        <h1 className={`text-2xl font-semibold ${th.txt}`}>{prop.addr}</h1>
                      ...
                      <p className={`${th.mut} mt-1`}>{prop.city}, {prop.state} {prop.zip}</p>
                      <p className={`text-xs ${th.mut}`}>ID: {prop.id}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
              ...
              <div className={`flex gap-1 ${th.card} rounded-xl border ${th.bdr} p-1.5`}>
                {['overview', 'tenants', 'history', 'expenses', 'insurance', 'loan', 'taxes', 'documents'].map(t => (
                            ...
                            return (
                              <div key={lease.id} className="space-y-3">
                                <div className="flex items-center gap-3">
                                  ...
                                  {tenant?.ri ? <ShieldCheck className="w-4 h-4 text-emerald-500" /> : <ShieldX className="w-4 h-4 text-red-500" />}
                                </div>
                          ...
                          <tbody className={`divide-y ${th.bdr}`}>
                            {propExpenses.map(exp => (
                              <tr key={exp.id} className={th.hov}>
                                ...
                                <td className={`px-5 py-3 text-sm ${th.mut}`}>{exp.method}</td>
                    ...
                    <div key={lease.id} className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                      <div className="flex items-start justify-between">
                        ...
                        <div className="text-right">
                          ...
                          {tenant?.ri ? <span className="inline-flex items-center gap-1 text-xs text-emerald-600"><ShieldCheck className="w-4 h-4" />Insured</span> : <span className="inline-flex items-center gap-1 text-xs text-red-600"><ShieldX className="w-4 h-4" />No Insurance</span>}
                        </div>
                      </div>
                      <div className={`grid grid-cols-5 gap-4 mt-4 pt-4 border-t ${th.bdr}`}>
                        ...
                        <div><p className={`text-xs ${th.mut}`}>Deposit</p><p className={`text-sm ${th.txt} mt-1`}>{fmt(lease.deposit)}</p></div>
                  ...
                  <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                    ...
                    {activeTenants.length > 0 ? activeTenants.map(t => (
                      <div key={t.id} className={`flex items-center justify-between p-4 rounded-lg ${t.ri ? 'bg-emerald-50' : 'bg-red-50'}`}>
                        <div className="flex items-center gap-3">
                          {t.ri ? <ShieldCheck className="w-5 h-5 text-emerald-600" /> : <ShieldX className="w-5 h-5 text-red-500" />}
            ...
            <div className="space-y-6">
              ...
              <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                ...
                <p className={`text-sm ${th.mut} mt-2`}>
                  <ShieldX className="w-4 h-4 text-red-500 inline mr-2" />
            ...
            <div className="space-y-6">
              ...
              <div className={`${th.card} rounded-xl border ${th.bdr} overflow-hidden`}>
                <table className="w-full">
                  ...
                  <tbody className={`divide-y ${th.bdr}`}>
                    {allExpenses.sort((a, b) => new Date(b.date) - new Date(a.date)).map(exp => {
                      ...
                      return (
                        <tr key={exp.id} className={th.hov}>
                          ...
                          <td className={`px-5 py-4 text-sm ${th.txt}`}>{exp.payee}</td>
                          <td className="px-5 py-4">
                            <p className={`text-sm ${th.txt}`}>{exp.description}</p>
                            {exp.notes && <p className={`text-xs ${th.mut} mt-0.5 italic`}>{exp.notes}</p>}
                          </td>
                          <td className={`px-5 py-4 text-sm ${th.mut}`}>{exp.method}</td>
                ...
                <div className="space-y-3">
                  {tenants.map(t => {
                    ...
                    <div key={t.id} className={`flex items-center justify-between p-4 rounded-xl ${t.ri ? 'bg-emerald-50' : 'bg-red-50'}`}>
                      <div className="flex items-center gap-4">
                        {t.ri ? <ShieldCheck className="w-6 h-6 text-emerald-600" /> : <ShieldX className="w-6 h-6 text-red-500" />}
      ...
      {showExpenseModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowExpenseModal(false)}>
          <div className={`${th.card} rounded-xl w-full max-w-lg shadow-2xl`} onClick={e => e.stopPropagation()}>
            <div className={`px-6 py-4 border-b ${th.bdr} flex justify-between items-center`}>
              <h3 className={`text-lg font-semibold ${th.txt}`}>Add Expense</h3>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select 
                    value={expenseForm.propId} 
                    onChange={e => setExpenseForm({ ...expenseForm, propId: e.target.value })}
                    className={`w-full px-4 py-2.5 rounded-lg border ${th.bdr} ${th.card} ${th.txt} text-sm`}
                  >
                    <option value="">Select property...</option>
                    {props.map(p => <option key={p.id} value={p.id}>{p.addr}</option>)}
                  </select>
                </div>
                <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    className={`w-full px-4 py-2.5 rounded-lg border ${th.bdr} ${th.card} ${th.txt} text-sm`}
                  >
                    {[...paymentMethods, 'Credit Card'].map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                </div>
              </div>
              <div>
            ...
            <div className={`px-6 py-4 border-t ${th.bdr} flex justify-end gap-3`}>
              ...
              <button onClick={recordExpense} className="px-5 py-2.5 rounded-lg bg-red-600 hover:bg-red-700 text-white text-sm font-medium">Add Expense</button>
            </div>
          </div>
        </div>
      )}
