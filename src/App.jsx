...
import { Home, Users, DollarSign, Shield, Plus, Search, ChevronRight, Edit2, AlertTriangle, CheckCircle, Clock, Building, Building2, Moon, Sun, ShieldCheck, ShieldOff, X, ChevronDown, Landmark, Receipt, Wrench, Star, Phone, Mail, MapPin, Trash2, Settings } from 'lucide-react';

const props = [
  { id: 1, addr: '123 Oak Street', city: 'Dallas', state: 'TX', zip: '75201', beds: 3, baths: 2, sqft: 1850, year: 1998, purchPrice: 245000, value: 310000, status: 'leased', loanId: 1 },
  { id: 2, addr: '456 Maple Avenue', city: 'Dallas', state: 'TX', zip: '75202', beds: 4, baths: 2.5, sqft: 2200, year: 2005, purchPrice: 320000, value: 385000, status: 'leased', loanId: 2 },
  { id: 3, addr: '789 Pine Road', city: 'Plano', state: 'TX', zip: '75023', beds: 3, baths: 2, sqft: 1650, year: 1992, purchPrice: 198000, value: 275000, status: 'leased', loanId: 3 },
  { id: 4, addr: '321 Elm Court', city: 'Frisco', state: 'TX', zip: '75034', beds: 4, baths: 3, sqft: 2600, year: 2015, purchPrice: 425000, value: 480000, status: 'vacant', loanId: 3 },
  { id: 5, addr: '555 Cedar Lane', city: 'McKinney', state: 'TX', zip: '75070', beds: 2, baths: 1, sqft: 1800, year: 1985, purchPrice: 175000, value: 245000, status: 'leased', loanId: 3 },
];
...
const leasesData = [
  ...
  { id: 1, propId: 1, tenantId: 6, start: '2020-01-15', end: '2022-12-31', rent: 1650, dueDay: 1, deposit: { amount: 1650, method: 'Check', paidDate: '2020-01-10', returned: true, returnedDate: '2023-01-15', returnedAmount: 1500, notes: 'Deducted $150 for carpet cleaning' }, status: 'ended' },
  { id: 2, propId: 1, tenantId: 1, start: '2023-04-01', end: '2025-03-31', rent: 1850, dueDay: 1, deposit: { amount: 1850, method: 'Zelle', paidDate: '2023-03-25', returned: false }, status: 'active' },
  // Property 2
  { id: 3, propId: 2, tenantId: 2, start: '2022-08-15', end: '2025-08-14', rent: 2400, dueDay: 15, deposit: { amount: 2400, method: 'Cashier Check', paidDate: '2022-08-10', returned: false }, status: 'active' },
  // Property 3 - three tenants over time, current tenant behind on rent
  { id: 4, propId: 3, tenantId: 7, start: '2019-06-01', end: '2021-05-31', rent: 1400, dueDay: 1, deposit: { amount: 1400, method: 'Money Order', paidDate: '2019-05-25', returned: true, returnedDate: '2021-06-15', returnedAmount: 1400, notes: 'Full refund, good tenant' }, status: 'ended' },
  { id: 5, propId: 3, tenantId: 6, start: '2021-07-01', end: '2023-06-30', rent: 1500, dueDay: 1, deposit: { amount: 1500, method: 'Zelle', paidDate: '2021-06-25', returned: true, returnedDate: '2023-07-10', returnedAmount: 1200, notes: 'Deducted $300 for wall repairs' }, status: 'ended' },
  { id: 6, propId: 3, tenantId: 3, start: '2023-08-01', end: '2025-07-31', rent: 1650, dueDay: 1, deposit: { amount: 1650, method: 'CashApp', paidDate: '2023-07-28', returned: false }, status: 'active' },
  // Property 4 - was leased, now vacant
  { id: 7, propId: 4, tenantId: 7, start: '2022-03-01', end: '2024-11-30', rent: 2800, dueDay: 1, deposit: { amount: 2800, method: 'Check', paidDate: '2022-02-20', returned: true, returnedDate: '2024-12-15', returnedAmount: 2100, notes: 'Deducted $700 for damages and cleaning' }, status: 'ended' },
  // Property 5 - duplex with two units
  { id: 8, propId: 5, tenantId: 4, unit: 'A', start: '2024-01-15', end: '2025-01-14', rent: 1100, dueDay: 15, deposit: { amount: 1100, method: 'Venmo', paidDate: '2024-01-10', returned: false }, status: 'active' },
  { id: 9, propId: 5, tenantId: 5, unit: 'B', start: '2023-06-01', end: '2025-05-31', rent: 1050, dueDay: 1, deposit: { amount: 1050, method: 'Zelle', paidDate: '2023-05-28', returned: false }, status: 'active' },
];
...
const paymentMethods = ['Zelle', 'CashApp', 'Venmo', 'Check', 'Direct Deposit', 'Zillow', 'Money Order', 'Cash', 'ACH', 'Wire', 'Credit Card', 'Other'];
...
export default function App() {
  ...
  const [paymentForm, setPaymentForm] = useState({ leaseId: '', amt: '', date: new Date().toISOString().split('T')[0], method: 'Zelle' });
  ...
  const recordPayment = () => {
    ...
    const lateFee = daysLate > 5 ? Math.round(parseFloat(paymentForm.amt) * 0.05) : 0; // 5% late fee after 5 days
    ...
    setPaymentForm({ leaseId: '', amt: '', date: new Date().toISOString().split('T')[0], method: 'Zelle' });
  };
    ...
    <div className={`h-screen flex ${th.bg}`} style={{ fontFamily: 'system-ui, sans-serif' }}>
      ...
      <aside className="w-64 bg-slate-900 text-white flex flex-col">
        ...
        <div className="p-4 border-t border-slate-800 space-y-1">
          <button onClick={() => setShowSettingsModal(true)} className="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm text-slate-300 hover:bg-slate-800">
      ...
      <main className="flex-1 overflow-y-auto p-8">
        <div className="max-w-7xl mx-auto">
          ...
          {view === 'properties' && !selId && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" />Add Property</button>
              </div>
            ...
            <div className="space-y-6">
                    ...
                    <div className={`${th.card} rounded-xl border ${th.bdr} overflow-hidden`}>
                      ...
                      {propExpenses.length > 0 ? (
                        <table className="w-full">
                          ...
                          <tbody className={`divide-y ${th.bdr}`}>
                            {propExpenses.map(exp => (
                              <tr key={exp.id} className={th.hov}>
                                ...
                                <td className="px-5 py-3">
                                  <button onClick={() => openExpenseModal(prop.id, exp)} className={`p-1.5 rounded-lg ${th.hov}`}><Edit2 className={`w-4 h-4 ${th.mut}`} /></button>
                                </td>
                              </tr>
                            ))}
                          </tbody>
                        ...
                        <div className="p-12 text-center">
                          ...
                          <p className={`${th.mut} mt-3`}>No expenses recorded</p>
                          <button onClick={() => openExpenseModal(prop.id)} className="mt-4 px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg text-sm font-medium">Add Expense</button>
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {tab === 'loan' && (() => {
                ...
                <div className={`${th.card} rounded-xl border ${th.bdr} p-12 text-center`}>
                  ...
                  <button className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">Add Loan</button>
                </div>
              );
              })()}
                ...
                <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center">
                      ...
                      <button className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" />Add Tenant</button>
                    </div>
                    {activeLeases.length > 0 ? activeLeases.map(lease => {
                      ...
                      <div className={`${th.card} rounded-xl border ${th.bdr} p-12 text-center`}>
                        ...
                        <button className="mt-4 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium">Add Tenant</button>
                      </div>
                    )}
                  </div>
          ...
          {view === 'tenants' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                <div><h1 className={`text-3xl font-semibold ${th.txt}`}>Tenants</h1><p className={`${th.mut} mt-1`}>{tenants.length} tenants</p></div>
                <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" />Add Tenant</button>
              </div>
              <div className={`${th.card} rounded-xl border ${th.bdr} overflow-hidden`}>
                <table className="w-full">
                  <thead><tr className={`${th.card2} text-xs ${th.mut} uppercase`}><th className="text-left px-5 py-4">Tenant</th><th className="text-left px-5 py-4">Property</th><th className="text-right px-5 py-4">Rent</th><th className="text-center px-5 py-4">Renters Insurance</th></tr></thead>
                  <tbody className={`divide-y ${th.bdr}`}>
                    {tenants.map(t => {
                      const p = props.find(x => x.id === t.propId);
                      return (
                        <tr key={t.id} className={th.hov}>
                          <td className="px-5 py-4"><div className="flex items-center gap-4"><div className="w-10 h-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-semibold text-sm">{t.name.split(' ').map(n => n[0]).join('')}</div><div><p className={`text-sm font-medium ${th.txt}`}>{t.name}</p><p className={`text-sm ${th.mut}`}>{t.email}</p></div></div></td>
                          <td className="px-5 py-4"><p className={`text-sm ${th.txt}`}>{p?.addr}</p><p className={`text-sm ${th.mut}`}>ID: {p?.id} · {p?.city}, {p?.state}</p></td>
                          <td className={`px-5 py-4 text-right text-sm font-semibold ${th.txt}`}>{fmt(t.rent)}</td>
                          <td className="px-5 py-4 text-center">{t.ri ? <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-emerald-100 text-emerald-700 font-medium"><ShieldCheck className="w-4 h-4" />Verified</span> : <span className="inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-red-100 text-red-700 font-medium"><ShieldOff className="w-4 h-4" />Missing</span>}</td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                <p className={`text-sm ${th.mut}`}>
                  <ShieldCheck className="w-4 h-4 text-emerald-500 inline mr-2" />
                  <strong className={th.txt}>Green checkmark</strong> = Tenant has renters insurance on file
                </p>
                <p className={`text-sm ${th.mut} mt-2`}>
                  <ShieldOff className="w-4 h-4 text-red-500 inline mr-2" />
                  <strong className={th.txt}>Red X</strong> = Tenant is missing renters insurance
                </p>
              </div>
            </div>
          )}
          ...
          {view === 'payments' && (
            <div className="space-y-6">
              <h1 className={`text-3xl font-semibold ${th.txt}`}>Payments</h1>
              <div className="grid grid-cols-3 gap-6">
                <Stat label="Collected" value={fmt(payments.filter(p => p.status === 'paid').reduce((s, p) => s + p.amt, 0))} color="text-emerald-600" />
                <Stat label="Pending" value={fmt(0)} color="text-amber-600" />
                <Stat label="Overdue" value={fmt(payments.filter(p => p.status === 'overdue').reduce((s, p) => s + p.amt, 0))} color="text-red-600" />
              </div>
              <div className={`${th.card} rounded-xl border ${th.bdr} overflow-hidden`}>
                <table className="w-full">
                  <thead><tr className={`${th.card2} text-xs ${th.mut} uppercase`}><th className="text-left px-5 py-4">Property</th><th className="text-left px-5 py-4">Tenant</th><th className="text-right px-5 py-4">Amount</th><th className="text-center px-5 py-4">Status</th></tr></thead>
                  <tbody className={`divide-y ${th.bdr}`}>
                    {payments.map(pay => {
                      const t = tenants.find(x => x.id === pay.tenantId);
                      const p = props.find(x => x.id === t?.propId);
                      return (
                        <tr key={pay.id} className={`${th.hov} ${pay.status === 'overdue' ? (dark ? 'bg-red-900/20' : 'bg-red-50') : ''}`}>
                          <td className="px-5 py-4"><p className={`text-sm font-medium ${th.txt}`}>{p?.addr}</p><p className={`text-sm ${th.mut}`}>ID: {p?.id}</p></td>
                          <td className={`px-5 py-4 text-sm ${th.txt}`}>{t?.name}</td>
                          <td className={`px-5 py-4 text-right text-sm font-semibold ${th.txt}`}>{fmt(pay.amt)}</td>
                          <td className="px-5 py-4 text-center"><span className={`inline-flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full font-medium ${pay.status === 'paid' ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-700'}`}>{pay.status === 'paid' ? <CheckCircle className="w-4 h-4" /> : <AlertTriangle className="w-4 h-4" />}{pay.status === 'paid' ? 'Paid' : `${pay.late}d Overdue`}</span></td>
                        </tr>
                      );
                    })}
                  </tbody>
              ...
              <div className={`${th.card} rounded-xl border ${th.bdr} overflow-hidden`}>
                <table className="w-full">
                  <thead><tr className={`${th.card2} text-xs ${th.mut} uppercase`}>
                    ...
                    <th className="text-left px-5 py-4">Payee</th>
                  ...
                  <tbody className={`divide-y ${th.bdr}`}>
                    {allExpenses.sort((a, b) => new Date(b.date) - new Date(a.date)).map((exp, idx) => {
                      const p = props.find(x => x.id === exp.propId);
                      const contractor = exp.contractorId ? contractors.find(c => c.id === exp.contractorId) : null;
                      return (
                        <tr key={exp.id} className={`${th.hov} cursor-pointer`} onClick={() => setSelectedExpense(exp.id)}>
                          <td className={`px-5 py-4 ${th.mut}`}>{idx + 1}</td>
                          <td className={`px-5 py-4 text-sm ${th.txt}`}>{fmtDate(exp.date)}</td>
                          <td className="px-5 py-4">
                            {p ? (
                              <>
                                <p className={`text-sm ${th.txt}`}>{p.addr}</p>
                                <p className={`text-xs ${th.mut}`}>{p.city}, {p.state} {p.zip}</p>
                              </>
                            ) : (
                              <span className={`text-sm ${th.mut}`}>Portfolio-wide</span>
                            )}
                          </td>
                          <td className="px-5 py-4">
                            <span className={`text-xs px-2.5 py-1 rounded-full whitespace-nowrap ${
                          ...
                          <td className="px-5 py-4">
                            <p className={`text-sm ${th.txt}`}>{exp.payee}</p>
                            {contractor && <p className={`text-xs ${th.mut}`}>{contractor.category}</p>}
                          </td>
                          <td className="px-5 py-4">
                            <p className={`text-sm ${th.txt}`}>{exp.description}</p>
                            {exp.notes && <p className={`text-xs ${th.mut} mt-0.5 italic`}>{exp.notes}</p>}
                          </td>
                          <td className={`px-5 py-4 text-sm ${th.mut}`}>
                            {exp.method}
                            {exp.checkNum && <span className="ml-1">#{exp.checkNum}</span>}
                          </td>
                          <td className={`px-5 py-4 text-sm font-semibold text-red-600 text-right`}>{fmt(exp.amount)}</td>
                          <td className="px-5 py-4" onClick={e => e.stopPropagation()}>
                            <button onClick={() => openExpenseModal(exp.propId, exp)} className={`p-1.5 rounded-lg ${th.hov}`}><Edit2 className={`w-4 h-4 ${th.mut}`} /></button>
                          </td>
                        </tr>
                      );
                    })}
          ...
          {/* Expense Detail View */}
          {view === 'expenses' && selectedExpense && (() => {
            const exp = allExpenses.find(e => e.id === selectedExpense);
            if (!exp) return null;
            const p = props.find(x => x.id === exp.propId);
            const contractor = exp.contractorId ? contractors.find(c => c.id === exp.contractorId) : null;
            return (
              <div className="space-y-6">
                <button onClick={() => setSelectedExpense(null)} className={`inline-flex items-center gap-2 text-sm ${th.mut} hover:${th.txt} transition`}>
                  <ChevronRight className="w-4 h-4 rotate-180" />
                  Back to Expenses
                </button>
                ...
                <div className={`${th.card} rounded-xl border ${th.bdr} p-6`}>
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <h1 className={`text-2xl font-semibold ${th.txt} mt-2`}>{exp.description || 'Expense'}</h1>
                      <p className={`text-3xl font-bold text-red-600 mt-2`}>{fmt(exp.amount)}</p>
                    </div>
                    <button onClick={() => openExpenseModal(exp.propId, exp)} className="px-4 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Edit2 className="w-4 h-4" />Edit Expense</button>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                    <h3 className={`text-sm font-medium ${th.txt} mb-4`}>Expense Details</h3>
                    <div className="space-y-4">
                      <div className="flex justify-between">
                        <span className={`text-sm ${th.mut}`}>Expense ID</span>
                        <span className={`text-sm font-medium ${th.txt}`}>{exp.id}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={`text-sm ${th.mut}`}>Date</span>
                        <span className={`text-sm font-medium ${th.txt}`}>{fmtDate(exp.date)}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={`text-sm ${th.mut}`}>Category</span>
                        <span className={`text-sm font-medium ${th.txt}`}>{exp.category}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className={`text-sm ${th.mut}`}>Payment Method</span>
                        <span className={`text-sm font-medium ${th.txt}`}>{exp.method}{exp.checkNum && ` #${exp.checkNum}`}</span>
                      </div>
                      {exp.receipt && (
                        <div className="flex justify-between">
                          <span className={`text-sm ${th.mut}`}>Receipt</span>
                          <span className="text-sm text-blue-600">{exp.receipt}</span>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                    <h3 className={`text-sm font-medium ${th.txt} mb-4`}>Property & Payee</h3>
                    <div className="space-y-4">
                      <div>
                        <span className={`text-xs ${th.mut}`}>Property</span>
                        {p ? (
                          <>
                            <button onClick={() => { setView('properties'); setSelId(p.id); setSelectedExpense(null); }} className={`block text-sm font-medium text-blue-600 hover:underline mt-1`}>{p.addr}</button>
                            <p className={`text-xs ${th.mut}`}>Property ID: {p.id}</p>
                          </>
                        ) : (
                          <p className={`text-sm ${th.mut} mt-1`}>Portfolio-wide expense</p>
                        )}
                      </div>
                      <div>
                        <span className={`text-xs ${th.mut}`}>Payee</span>
                        {contractor ? (
                          <button onClick={() => { setView('contractors'); setSelectedContractor(contractor.id); setSelectedExpense(null); }} className={`block text-sm font-medium text-blue-600 hover:underline mt-1`}>{exp.payee}</button>
                        ) : (
                          <p className={`text-sm font-medium ${th.txt} mt-1`}>{exp.payee}</p>
                        )}
                        {contractor && <p className={`text-xs ${th.mut}`}>{contractor.category} · {contractor.phone}</p>}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                {exp.notes && (
                  <div className={`${th.card} rounded-xl border ${th.bdr} p-5`}>
                    <h3 className={`text-sm font-medium ${th.txt} mb-2`}>Notes</h3>
                    <p className={`text-sm ${th.mut}`}>{exp.notes}</p>
                  </div>
                )}
              </div>
            );
          ...
          {view === 'contractors' && selectedContractor && (() => {
            ...
            <div className="space-y-6">
              ...
              <div className={`${th.card} rounded-xl border ${th.bdr} p-6`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-6">
                    <div className={`w-16 h-16 rounded-xl flex items-center justify-center ${c.active ? 'bg-blue-600' : 'bg-gray-400'}`}>
                      <Wrench className="w-8 h-8 text-white" />
                    </div>
                    <div>
                      <div className="flex items-center gap-3">
                        <h1 className={`text-2xl font-semibold ${th.txt}`}>{c.name}</h1>
                        <span className={`text-xs px-3 py-1 rounded-full font-medium ${c.active ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-600'}`}>{c.active ? 'Active' : 'Inactive'}</span>
                      </div>
                      <div className="flex items-center gap-3 mt-2">
                        <span className={`text-sm px-3 py-1 rounded-full ${dark ? 'bg-slate-700 text-slate-300' : 'bg-gray-100 text-gray-600'}`}>{c.category}</span>
                        <div className="flex items-center gap-0.5">
                          {[...Array(5)].map((_, i) => (
                            <Star key={i} className={`w-4 h-4 ${i < c.rating ? 'text-amber-400 fill-amber-400' : th.mut}`} />
                          ))}
                        </div>
                      </div>
                    </div>
                  </div>
                  <button onClick={() => openContractorModal(c)} className={`px-4 py-2.5 rounded-lg border ${th.bdr} ${th.txt} text-sm font-medium ${th.hov} flex items-center gap-2`}><Edit2 className="w-4 h-4" />Edit</button>
                </div>
                <div className={`grid grid-cols-4 gap-6 mt-6 pt-6 border-t ${th.bdr}`}>
                  <div>
                    <p className={`text-xs ${th.mut}`}>Phone</p>
                    <p className={`text-sm ${th.txt} mt-1`}>{c.phone}</p>
                  </div>
                  <div>
                    <p className={`text-xs ${th.mut}`}>Email</p>
                    <p className="text-sm text-blue-600 mt-1">{c.email}</p>
                  </div>
                  <div>
                    <p className={`text-xs ${th.mut}`}>Address</p>
                    <p className={`text-sm ${th.txt} mt-1`}>{c.address || '—'}</p>
                  </div>
                  <div>
                    <p className={`text-xs ${th.mut}`}>Notes</p>
                    <p className={`text-sm ${th.txt} mt-1`}>{c.notes || '—'}</p>
                  </div>
                </div>
              </div>
              ...
              {/* Stats */}
              <div className="grid grid-cols-4 gap-6">
                <Stat label="Total Paid" value={fmt(totalPaid)} color="text-red-600" />
                <Stat label="Expenses" value={contractorExpenses.length} color="text-blue-600" />
                <Stat label="Properties Serviced" value={[...new Set(contractorExpenses.map(e => e.propId).filter(Boolean))].length} color="text-violet-600" />
                <Stat label="Avg per Job" value={contractorExpenses.length > 0 ? fmt(totalPaid / contractorExpenses.length) : '$0'} color="text-amber-600" />
              </div>
          ...
          {view === 'financing' && (() => {
            ...
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" />Add Loan</button>
              </div>
          ...
          {view === 'insurance' && (
            <div className="space-y-6">
              <div className="flex justify-between items-center">
                ...
                <button className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium flex items-center gap-2"><Plus className="w-4 h-4" />Add Policy</button>
              </div>
              <div className="grid grid-cols-4 gap-6">
      ...
      {showSettingsModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setShowSettingsModal(false)}>
          <div className={`${th.card} rounded-xl w-full max-w-lg shadow-2xl flex flex-col max-h-[80vh]`} onClick={e => e.stopPropagation()}>
            <div className={`px-6 py-4 border-b ${th.bdr} flex justify-between items-center flex-shrink-0`}>
              <h3 className={`text-lg font-semibold ${th.txt}`}>Settings</h3>
              <button onClick={() => setShowSettingsModal(false)} className={`p-1.5 rounded-lg ${th.hov}`}><X className="w-5 h-5" /></button>
            </div>
            <div className="p-6 space-y-6 overflow-y-auto flex-1">
              {/* Expense Categories */}
              <div>
                <h4 className={`text-sm font-medium ${th.txt} mb-3`}>Expense Categories</h4>
                <div className="flex gap-2 mb-3">
                  <input 
                    ...
                    className={`flex-1 px-4 py-2 rounded-lg border ${th.bdr} ${th.card} ${th.txt} text-sm`}
                  />
                  <button 
                    onClick={() => {
                ...
                <div className="space-y-1">
                  {expenseCategories.map((cat, idx) => (
                    <div key={idx} className={`flex items-center justify-between px-3 py-2 rounded-lg ${th.card2}`}>
                      <span className={`text-sm ${th.txt}`}>{cat}</span>
                      <button 
                        onClick={() => {
              ...
              <div>
                <h4 className={`text-sm font-medium ${th.txt} mb-3`}>Contractor Categories</h4>
                <p className={`text-xs ${th.mut} mb-2`}>Plumbing, HVAC, Electrical, Roofing, Landscaping, Painting, Flooring, Security, Pest Control, General</p>
                <p className={`text-xs ${th.mut} italic`}>Contact support to modify contractor categories.</p>
              </div>
            </div>
            <div className={`px-6 py-4 border-t ${th.bdr} flex justify-end flex-shrink-0`}>
              <button onClick={() => setShowSettingsModal(false)} className="px-5 py-2.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium">Done</button>
            </div>
          </div>
        </div>
      )}
